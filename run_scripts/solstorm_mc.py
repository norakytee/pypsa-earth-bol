...
def run_monte_carlo_simulation(iteration, all_indices, datetime_column, line_lengths):
    ...
    df_lines.to_csv('/mnt/beegfs/users/noraky/pypsa-earth/matrices/lines_matrix.csv', index=False)
    df_generators.to_csv('/mnt/beegfs/users/noraky/pypsa-earth/matrices/generators_matrix.csv', index=False)
    df_storage_units.to_csv('/mnt/beegfs/users/noraky/pypsa-earth/matrices/storage_units_matrix.csv', index=False)
    
    # Saving matrix file for all iterations
    df_lines.to_csv('/mnt/beegfs/users/noraky/pypsa-earth/matrices/lines_matrix_' + str(iteration) + '.csv', index=False)
    df_generators.to_csv('/mnt/beegfs/users/noraky/pypsa-earth/matrices/generators_matrix_' + str(iteration) + '.csv', index=False)
    df_storage_units.to_csv('/mnt/beegfs/users/noraky/pypsa-earth/matrices/storage_units_matrix_' + str(iteration) + '.csv', index=False)
...
def rename_results_file(iteration):
    ...
    new_location = '/mnt/beegfs/users/noraky/pypsa-earth/monte_carlo_networks_emissions/'
...
def yearly_changes(n,year):
    ...
    solved_network = '/mnt/beegfs/users/noraky/pypsa-earth/network_bnze_2040/BNZE_2040.nc'
...
for iteration in range(n_simulations):
    n = pypsa.Network('/mnt/beegfs/users/noraky/pypsa-earth/network_bnze_2040/network_bnze_2040.nc')
