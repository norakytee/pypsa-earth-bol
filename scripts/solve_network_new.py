...
# def new_cp_constraint(n):
#     """Adds a yearly constraint to the capacity factors of the generators."""
#     cp = 0.6
#     hours_in_year = 8760
#     ccgt_i = n.generators.query("carrier == 'CCGT'").index
#     ccgt_p = get_var(n, "Generator", "p").loc[n.snapshots[:], ccgt_i]
#     lhs = linexpr((1,ccgt_p)).sum().sum()
#     rhs = n.generators.loc[ccgt_i,'p_nom'].sum() * hours_in_year * cp
#     define_constraints(n, lhs, "<=", rhs, "CCGT", "cp_constraint")

# def new_cp_constraint_bio(n):
#     """Adds a yearly constraint to the capacity factors of the generators."""
#     cp = 0.72
#     hours_in_year = 8760
#     bio_i = n.generators.query("carrier == 'biomass'").index
#     bio_p = get_var(n, "Generator", "p").loc[n.snapshots[:], bio_i]
#     lhs = linexpr((1,bio_p)).sum().sum()
#     rhs = n.generators.loc[bio_i,'p_nom'].sum() * hours_in_year * cp
#     define_constraints(n, lhs, "<=", rhs, "CCGT", "cp_constraint")
    
# def new_cp_constraint_hydro(n):
#     """Adds a yearly constraint to the capacity factors of the generators."""
#     cp = 0.5
#     hours_in_year = 8760
#     ror_i = n.generators.query("carrier == 'ror'").index
#     ror_p = get_var(n, "Generator", "p").loc[n.snapshots[:], ror_i]
#     lhs = linexpr((1,ror_p)).sum().sum()
#     rhs = n.generators.loc[ror_i,'p_nom'].sum() * hours_in_year * cp
#     define_constraints(n, lhs, "<=", rhs, "CCGT", "cp_constraint")

def apply_cp_constraints(n):
    """Adds yearly constraints to the capacity factors of the generators for different carriers."""
    hours_in_year = 8760
    carrier_cp = {
        'CCGT': 0.64,
        'OCGT': 0.64,
